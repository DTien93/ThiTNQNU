<?php
   ...
   $kt1= "select * from giaovien where taikhoan= '".$ten."'";
    ...
    $result2 = mysqli_query($conn,$kt1) or die ("Không thể kết nối được !");
   ...
   if($row = mysqli_fetch_array($result))
   {
     if($ten == $row['taikhoan'])
     {
       if($pass == $row['matkhau'])
       {
         ...
         header('location:http://localhost:70/ThiTN/Manager/index.php');
       ...
       {
        echo "<script>alert('Mật khẩu không hợp lệ!'); window.location='http://localhost:70/ThiTN/Manager/Logins/Login/index.html'</script>";
       }
      }
     ...
     if($row1 = mysqli_fetch_array($result1))
     {
       if($ten == $row1['taikhoan'])
       {
          if($pass == $row1['matkhau'])
          {
            ...
            header('location:http://localhost:70/ThiTN/Teacher/home.php');
          }
          else
          {
            echo "<script>alert('Mật khẩu không hợp lệ!'); window.location='http://localhost:70/ThiTN/Manager/Logins/Login/index.html'</script>";
      ...
      if($row2 = mysqli_fetch_array($result2))
      {
        if($ten == $row2['taikhoan'])
        {
           ...
           {
             $_SESSION['taikhoan'] = $row1['taikhoan'];
             header('location:http://localhost:70/ThiTN/Admin/qlgv/gv.php');
           }
           else
           {
             echo "<script>alert('Mật khẩu không hợp lệ!'); window.location='http://localhost:70/ThiTN/Manager/Logins/Login/index.html'</script>";
   ...
   else if ($row1 = mysqli_fetch_array($result1)) 
   {
     if($ten == $row1['taikhoan'])
     {
       ...
       {
        echo "<script>alert('Mật khẩu không hợp lệ!'); window.location='http://localhost:70/ThiTN/ManagerStudent/Logins/Login/index.html'</script>";
       }
      }   

     ...
     if($row1 = mysqli_fetch_array($result1))
     {
       if($ten == $row1['taikhoan'])
       {
          if($pass == $row1['matkhau'])
          {
            ...
            header('location:http://localhost:8080/ThiTN/Teacher/home.php');
          }
          else
          {
            echo '<script type="text/javascript">'; 
            echo 'if(confirm("Bạn đã nhập sai mật khẩu.")){';    
            echo 'window.location="http://localhost:8080/ThiTN/Login/login.html";}';  
            echo 'else { window.location="http://localhost:8080/ThiTN/Login/login.html";}';   
            echo '</script>';
          }
        }
      }

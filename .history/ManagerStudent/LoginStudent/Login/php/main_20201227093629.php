<?php
   ...
   if($row = mysqli_fetch_array($result))
   {
     if($ten == $row['taikhoan'])
     {
       ...
       {
        echo "<script>alert('Mật khẩu không hợp lệ!'); window.location='http://localhost:70/ThiTN/ManagerStudent/LoginStudent/Login/index.html'</script>";
       }
      }
     ...
     if($row1 = mysqli_fetch_array($result1))
     {
       if($ten == $row1['taikhoan'])
       {
          ...
          else
          {
            ...
            echo 'if(confirm("Bạn đã nhập sai mật khẩu.")){';    
            echo 'window.location="http://localhost:70/ThiTN/Teacher/Login/login.html";}';  
            echo 'else { window.location="http://localhost:70/ThiTN/Login/login.html";}';   
            echo '</script>';
          }
        }
      }

<?php
      ...
      $classhs = $_POST["class"];
      $khoihs = $_POST["block"];
      $addresshs = $_POST["address"];
      $emailhs = $_POST["email"];
      $datehs = $_POST["date"];
      $hometownhs = $_POST["hometown"];

      $connect = mysqli_connect("localhost", "root", "", "tttracnghiem");

      if (isset($_POST['update'])) {
        $query = "update hosinh set hoTen = '".$tenhs."',  lop = '".$classhs."', khoi = '".$khoihs."' , diaChi = '".$addresshs."', Email = '".$emailhs."',
        ngaySinh  = '".$datehs."', queQuan = '".$hometownhs."'  where mahs = '".$mahs."'";

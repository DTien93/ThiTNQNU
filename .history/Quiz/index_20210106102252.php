...
$('#btnNote').click(function() {
    ...
    function checkResult() {  
        ...
        let score = 0;
          $('#questions div.row').each(function(k, v){
              let dapandung = $(v).find('h5').attr('dapandung');
              let question = questions.find(x=>x.dapandung === dapandung);
              ...
              $('#question_'+dapandung+' > fieldset > div > label.'+answer).css("background-color", "orange") ;
          });      
    ...
    success:function(data){
      ...
      $.each(questions,function(k,v){
        d+=   '<div class="row" style = "margin-left:10px;" id="question_'+v['dapandung']+'"> ';
        d+=   '<h5 style="font-weight:bold;" id="'+v['id']+'"> <span class="text-danger">Câu '+index+': </span>'+v['cauhoi']+'</h5>';
        d +=   '<fieldset>';
        d+=   '<div class="radio col-md-12 ">';
        d+=    '<label class = "A"><input type="radio" class="A" name = "'+v['dapandung']+'"><span class="text-danger">A: </span>'+v['dapan1']+'</label>';
        d+=   '</div>';
        d+=  ' <div class="radio col-md-12">';
        d+=     '<label class = "B"><input type="radio" class="B" name = "'+v['dapandung']+'"><span class="text-danger">B: </span>'+v['dapan2']+'</label>';
        d+=   '</div>';
        d+=   '<div class="radio  col-md-12">';
        d+=     '<label class = "C"><input type="radio"  class="C" name = "'+v['dapandung']+'"><span class="text-danger">C: </span>'+v['dapan3']+'</label>';
        d+=   '</div>';
        d+=   '<div class="radio col-md-12">';
        d+=     '<label class = "D"><input type="radio" class="D" name = "'+v['dapandung']+'"><span class="text-danger">D: </span>'+v['dapan4']+'</label>';
        d+=   '</div>';
        d+=  '</fieldset>';
        d+=  '</fieldset>';
        d+= '</div>';

...
$('#btnNote').click(function() {
    ...
    function checkResult() {  
        ...
        let score = 0;
          $('#questions div.row').each(function(k, v){
              let id = $(v).find('h5').attr('idcauhoi'); // lay thuoc tinh id cua the h5 ra 
              let question = questions.find(x=>x.id == id);// tim cau hoi trong mang question vao id da co tren
              let answer = question['dapan4'];
    ...
    success:function(data){
      ...
      $.each(questions,function(k,v){
        ...
        d+=   '</div>';
        d+=   '<div class="radio col-md-12">';
        d+=     '<label class = "D"><input type="radio" class="D" name = "'+v['dapan4']+'"><span class="text-danger">D: </span>'+v['dapan4']+'</label>';
        d+=   '</div>';
        d+=  '</fieldset>';
        d+=  '</fieldset>';
        d+= '</div>';

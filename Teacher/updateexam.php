<?php
	...
	if(isset($_POST["update"]))
	{
		...
		if($ngayThi!=$row['ngaythi'] || $thoiLuong!=$row['thoiluong']){
			...
			{
				echo "<script>alert('Cập nhật kỳ thi thành công.'); window.location='http://localhost:70/ThiTN/Teacher/examinformation.php'</script>";
			}
		}
		...
		{
			echo "<script>alert('Thông tin kỳ thi không thay đổi!.'); window.location='http://localhost:70/ThiTN/Teacher/examinformation.php'</script>";
		}
	}
	mysqli_close($conn);
?>